...
import { getCourseVideo, showCourseList } from '@/api/course'
...
/**视频跳转 */
const videobofang=async(item:CourseItemInfo)=>{
  userStore.setCourse(item.id)
  userStore.setCourseinfo(item)
  router.push('/user/coursevideo')
}
...
<template>
  <div class="course_list_page">
    <el-carousel :interval="3000" type="card" height="200px">
      <el-carousel-item v-for="item in 3" :key="item">
        <h3 text="2xl" justify="center">
          <img :src="`/src/assets/class/banner${item}.png`" alt="" style="width: 100%;height: 100%;"/>
        </h3>
      </el-carousel-item>
    </el-carousel>
    <h3>好课推荐</h3>
    <div class="course_list">
      <el-card
        style="width: 35%; min-height: 200px"
        shadow="hover"
        v-for="item in courseListItemfit"
        :key="item.id"
        :body-style="{ padding: 0 }"
        @click="videobofang(item)"
      >
        <img :src="`/src/assets/class/${item.id}.png`" alt="" />
        <div class="details">
